...
(() => {
      ...
      loginForm.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          if (data.success) {
            ...
            errorMessageEl.textContent = data.message || 'Erro no login. Tente novamente.';
          }
        } catch (error) {
          console.error('Erro de rede ou servidor:', error);
          errorMessageEl.textContent = 'Erro ao conectar com o servidor. Tente novamente.';
        }
      });
    }
  ...
  if (document.body.classList.contains('page-index')) {
    ...
    const encontroSelectionModal = document.getElementById('encontroSelectionModal');
    const closeEncontroSelectionButton = encontroSelectionModal.querySelector('.close-button');
    const encontrosRadioButtonsDiv = document.getElementById('encontrosRadioButtons');
    const confirmSelectionBtn = document.getElementById('confirmSelectionBtn');
    const encontroSelectionModalTitle = encontroSelectionModal.querySelector('h2'); // Adicionado para mudar o título do modal

    // Modal para visualização de faltas (o "olhinho")
    const faltasDetailsModal = document.getElementById('faltasDetailsModal');
    const closeFaltasDetailsModalButton = document.getElementById('closeFaltasDetailsModal');
    const crismandoNomeFaltasSpan = document.getElementById('crismandoNomeFaltas');
      ...
      alunos.forEach((aluno, i) => {
        ...
        tbody.insertAdjacentHTML('beforeend', `
          <tr data-id="${aluno.id}" class="${linhaClass}">
            <td>${aluno.nome}</td>
            <td>
              <button class="icon diminuir-faltas" data-crismando-id="${aluno.id}" ${aluno.faltas <= 0 ? 'disabled' : ''}>-</button>
              <span class="faltas-valor">${aluno.faltas}</span> <button class="icon aumentar-faltas" data-crismando-id="${aluno.id}">+</button>
            </td>
            <td>${Math.max(0, presencasCalculadas)}</td>
            <td>
              <button class="icon view-faltas" data-crismando-id="${aluno.id}">👁️</button>
              <button class="icon edit-crismando" data-crismando-id="${aluno.id}">✏️</button>
              <button class="icon del-crismando" data-crismando-id="${aluno.id}">🗑️</button>
            </td>
          </tr>
        `);
      });
    ...
    async function fetchAllEncontrosAndPopulateModal() {
        encontrosRadioButtonsDiv.innerHTML = '';
        if (encontros.length === 0) {
            encontrosRadioButtonsDiv.innerHTML = '<p>Nenhum encontro cadastrado. Cadastre encontros na página de Encontros.</p>';
            ...
            encontros.forEach(encontro => {
                ...
                `;
                encontrosRadioButtonsDiv.appendChild(label);
            });
        }
    }
    ...
    async function fetchFaltasDoCrismandoAndPopulateModal(crismandoId) {
        ...
        try {
            ...
            if (faltasDoCrismando.length === 0) {
                encontrosRadioButtonsDiv.innerHTML = '<p>Este crismando não possui faltas registradas individualmente que possam ser removidas.</p>';
            ...
            encontrosRadioButtonsDiv.innerHTML = `<p>Erro ao carregar faltas para remoção: ${error.message}. Tente novamente.</p>`;
  ...
  try {
    ...
    if (id) {
      ...
      response = await fetch(`${API_BASE_URL}/crismandos`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome, faltas, presencas: encontros.length - faltas })
      });
    }
    ...
    tbody.addEventListener('click', async (event) => {
      ...
      if (target.classList.contains('aumentar-faltas')) {
        ...
        encontroSelectionModalTitle.textContent = 'Selecione o Encontro para Adicionar Falta'; // Título para adição
        await fetchAllEncontrosAndPopulateModal(); // Mostra TODOS os encontros
        encontroSelectionModal.style.display = 'block';
      } else if (target.classList.contains('diminuir-faltas')) {
        currentCrismandoId = crismandoId;
        currentActionType = 'removeFalta';
        encontroSelectionModalTitle.textContent = 'Selecione a Falta para Remover'; // Título para remoção
    ...
    // Confirmar seleção de encontro no modal (para adicionar/remover falta)
    ...
    async function displayFaltasDetails(crismandoId, crismandoNome) {
      ...
      try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`Erro ao carregar detalhes das faltas: ${response.status} - ${errorText}`);
        }
        ...
        if (faltas.length === 0) {
          faltasListDiv.innerHTML = '<p>Este crismando não possui faltas registradas individualmente.</p>';
        } else {
          ...
          faltas.forEach(falta => {
            ...
            li.textContent = `Assunto: ${falta.assunto || 'N/A'}, Data: ${dataFaltaFormatada || 'N/A'}, Local: ${falta.local || 'N/A'}`;
            ul.appendChild(li);
          });
          faltasListDiv.appendChild(ul);
        }
      } catch (error) {
        console.error('Erro ao buscar detalhes das faltas para o olhinho:', error);
    ...
    generateReportBtn.addEventListener('click', async () => {
      try {
        ...
        alert('Relatório PDF gerado e download iniciado/concluído com sucesso!');
  ...
  if (document.body.classList.contains('page-encontros')) {
    ...
    tbody.addEventListener('click', async (event) => {
      ...
      if (target.classList.contains('edit')) {
        ...
        const formattedTime = encontroDate.toTimeString().split(' ')[0].substring(0, 5);
