...
db.connect()
    ...
    .catch(err => console.error('Erro na conexão com o PostgreSQL:', err.stack));
...
app.get('/api/crismandos', async (req, res) => {
    try {
        const result = await db.query('SELECT * FROM crismandos ORDER BY nome ASC');
...
app.get('/api/crismandos/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.query('SELECT * FROM crismandos WHERE id = $1', [id]);
...
app.put('/api/crismandos/:id', async (req, res) => {
    const { id } = req.params;
    const { nome, faltas, presencas } = req.body;
    try {
        const result = await db.query(
            'UPDATE crismandos SET nome = $1, faltas = $2, presencas = $3 WHERE id = $4 RETURNING *',
            [nome, faltas, presencas, id]
        );
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Crismando não encontrado' });
        }
...
app.delete('/api/crismandos/:id', async (req, res) => {
    ...
    try {
        ...
        await db.query('DELETE FROM faltas_crismandos WHERE crismando_id = $1', [id]);
        // Em seguida, deletar o crismando
        const result = await db.query('DELETE FROM crismandos WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Crismando não encontrado' });
        }
...
app.post('/api/faltas', async (req, res) => {
    ...
    try {
        ...
        if (existingFalta.rows.length > 0) {
            return res.status(409).json({ error: 'Falta para este encontro já registrada para este crismando.' });
        }
        ...
        res.status(201).json({ message: 'Falta registrada com sucesso!', crismando: updatedCrismando.rows[0] });
...
app.get('/api/encontros/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.query('SELECT * FROM encontros WHERE id = $1', [id]);
        if (result.rows.length === 0) {
...
app.delete('/api/encontros/:id', async (req, res) => {
    ...
    try {
        ...
        await db.query('DELETE FROM faltas_crismandos WHERE encontro_id = $1', [id]);
        // Em seguida, deletar o encontro
        const result = await db.query('DELETE FROM encontros WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Encontro não encontrado' });
        }
        res.json({ message: 'Encontro e faltas associadas deletados com sucesso!' });
    } catch (err) {
        console.error(err);
...
app.get('/api/report/pdf', async (req, res) => {
    try {
        ...
        const alunosResult = await db.query('SELECT * FROM crismandos ORDER BY nome ASC');
        const encontrosResult = await db.query('SELECT * FROM encontros ORDER BY data ASC');
        ...
        const faltasPorCrismando = faltasDetalhesResult.rows.reduce((acc, falta) => {
        ...
        doc.font('Helvetica-Bold')
            .fontSize(18)   
            .text('Relatório de Crismandos - Capela N. S. Aparecida', { align: 'center' });
        ...
        doc.font('Helvetica')
            .fontSize(12)
            .text(`${dataGeracaoRelatorioFormatada}`);

        doc.text('____________________________________________________________________________').moveDown(1.5);

        // Informações sobre Encontros
        doc.fontSize('Helvetica-Bold')
            .fontSize(15)
            .text('Encontros Cadastrados', { underline: true }).moveDown(0.5);
        ...
        if (encontros.length === 0) {
            ...
            encontros.forEach((encontro) => {
                const dataFormatada = new Date(encontro.data).toLocaleDateString('BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
                doc.font('Helvetica-Bold')
                    .fontSize(12)
                ...
                doc.font('Helvetica')
                    .fontSize(12)
                    .text(`${encontro.assunto}`).moveDown(0.2);
                ...
                doc.font('Helvetica')
                .fontSize(12)
                    .text(`${encontro.local}`).moveDown(0.2);
                ...
                doc.font('Helvetica')
                    .fontSize(12)
                    .text(`${dataFormatada}`).moveDown(0.8);
            });
        }
        doc.moveDown();
        ...
        doc.font('Helvetica')
            .fontSize(15)
            .text('Crismandos Cadastrados', { underline: true }).moveDown(0.5);
        ...
        if (alunos.length === 0) {
            ...
            for (const aluno of alunos) {
                ...
                doc.font('Helvetica')
                    .fontSize(12)
                    .text(`${Math.max(0, presencasCalculadas)}`).moveDown(0.2); // Small moveDown
                ...
                doc.font('Helvetica')
                    .fontSize(12)
                    .text(`${aluno.faltas}`).moveDown(0.2); // Small moveDown
